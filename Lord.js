...
_L O A D I N G_`
						...
						case 'asupan':{
									...
									Zeeone.sendMessage(from, p, video, { quoted:  Ofc, mimetype: 'video/mp4', caption: 'NIH'})
									limitAdd(sender, limit)
									}
									break
...
Alasan : ${reason}`, [sender], true)
						...
						case 'sewabot': 
									let gakloo = [{
										...
										break
						case 'donasi':
									reply('GOPAY/DANA : 089516833542 , SEBAIKNYA CHAT OWNER DULU SEBELUM MELAKUKAN TRANSAKSI')
									break
						...
						case 'setnamagrup': case 'setnamegrup': case 'setgrupname':
									...
									if (!isGroupAdmins && !isOwner) return reply(mess.only.admin)
									if (!isBotGroupAdmins) return reply(`Jadikan Bot Sebagai Admin Group!`)
									if (args.length == 0) return reply(`Penggunaan ${prefix + command} Arya4Me Support`)
						...
						case 'cekprem': case 'cekpremium': case 'premiumcek':
									...
									reply(`${isOwner?'Unlimited Prem (Lu Owner Arya4Me)':premiumnya}`)
									break
...
💹 *EXPIRE :* ${cekvip.days} day(s) ${cekvip.hours} hour(s) ${cekvip.minutes} minute(s)`
						...
						case 'antilink':
									if (!isGroupAdmins && !isOwner && !Ofc.key.fromMe) return reply(mess.only.admin)
										if (args[0].toLowerCase() === 'on'){
											if (isAntiLink) return reply(`DAH AKTIF BRO!`)
											antilink.push(from)
						...
						case 'clearbadword':
									...
									reply(`S U C C E S S Cler Badword Si ${entah}`)
									break
						case 'antitroli': case 'antifaketroli':{
									try {
										if (!isGroupAdmins && !isOwner && !Ofc.key.fromMe) return reply(mess.only.admin)
										if (args[0].toLowerCase() === 'on'){
											if (isAntro) return reply(`DAH AKTIF SEBELUMNYA!`)
											atro.push(from)
...
\`\`\`${groupDesc}\`\`\``
						...
						case 'toimg': 
									...
									if ((isMedia && !Ofc.message.videoMessage || isQuotedImage || isQuotedSticker && args.length == 0)) {
										...
										exec(`ffmpeg -i ${media} ${ran}`, (err) => {
											...
											buffer = fs.readFileSync(ran)
											Zeeone.sendMessage(from, buffer, image, {thumbnail : thumbnail, caption:'S U C C E S S',quoted:Ofc})
											fs.unlinkSync(ran)
											})
										} else {
						...
						case 'togif':
									...
									Zeeone.sendMessage(from, Frd, video, { mimetype: 'video/gif', caption: 'S U C C E S S', quoted: Ofc })
									limitAdd(sender, limit)
									break
...
*🔖 Url : ${vrr.display_url}*`
						...
						case 'ban':
									...
									if (Ofc.message.extendedTextMessage === undefined || Ofc.message.extendedTextMessage === null) {
										...
										reply(`S U C C E S S Ban Nomor ${ny}, Mampus Lu!`)
										} else {
											ny = `${mentionUser}`
											ban.push(ny)
											fs.writeFileSync('./database/banned.json', JSON.stringify(ban))
						...
						case 'unban':
									...
									if (Ofc.message.extendedTextMessage === undefined || Ofc.message.extendedTextMessage === null) {
										...
										ban.splice(unb, 1)
										fs.writeFileSync('./database/banned.json', JSON.stringify(ban))
										reply(`S U C C E S S Unban Nomor ${ah}`)
										} else {
											ah = `${mentionUser}`
											unb = ban.indexOf(ah)
											ban.splice(unb, 1)
						...
						case 'bc': case 'broadcast':
									if (!Ofc.key.fromMe && !isOwner) return reply(mess.only.owner) 
									if (args.length < 1) return reply(`${prefix + command} Arya4Me`)
							...
							for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
								...
								ini_list.push({
									...
									hehe = await Zeeone.sendMessage(from, {
										...
										Zeeone.sendMessage(from, `Owner Arya4Me SPAM/CHAT GAJELAS = BLOCK`, text, {quoted: hehe})
										}
										break
						case 'lirik':
...
let qqppp = [{
                ...
                sendButLocation(from, teks, `Arya4Me™© | Allright Reserved.`, thumbnail, qqppp, {contextInfo: { mentionedJid: [nupe]}})
...
break
						...
						case 'nuliskiri':{
									...
									.on('exit', () => {
										Zeeone.sendMessage(from, fs.readFileSync('./media/nulis/images/buku/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: Ofc, caption: `S U C C E S S ರ_ರ`})
										limitAdd(sender, limit)
										})
									}
									break
						case 'nuliskanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										Zeeone.sendMessage(from, fs.readFileSync('./media/nulis/images/buku/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: Ofc, caption: `S U C C E S S ರ_ರ`})
										limitAdd(sender, limit)
										})
									}
									break
						case 'foliokiri':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										Zeeone.sendMessage(from, fs.readFileSync('./media/nulis/images/folio/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: Ofc, caption: `S U C C E S S ರ_ರ`})
										limitAdd(sender, limit)
										})
									}
									break
						case 'foliokanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										Zeeone.sendMessage(from, fs.readFileSync('./media/nulis/images/folio/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: Ofc, caption: `S U C C E S S ರ_ರ`})
										limitAdd(sender, limit)
									})
									}
									break
						...
						case 'delaudio': case 'delvn': case 'deletevn':
									...
									try {
										...
										fs.writeFileSync('./database/audio.json', JSON.stringify(audionye))
										reply(`S U C C E S S Menghapus Audio ${q}!`)
										await sleep(5000)
										} catch (err) {
											console.log(err)
						...
						case 'delimage': case 'deleteimage': case 'delfoto': case 'delphoto':
									...
									try {
										...
										fs.writeFileSync('./database/image.json', JSON.stringify(imagenye))
										reply(`S U C C E S S Menghapus Image ${q}!`)
										await sleep(5000)
										} catch (err) {
						...
						case 'delvideo': case 'deletevideo':
									...
									try {
										...
										fs.writeFileSync('./database/video.json', JSON.stringify(videonye))
										reply(`S U C C E S S Menghapus Video ${q}!`)
										await sleep(5000)
										} catch (err) {
						...
						case 'delstik': case 'delstick': case 'delsticker': case 'delstiker':
									...
									try {
										...
										fs.writeFileSync('./database/stick.json', JSON.stringify(setiker))
										reply(`S U C C E S S Menghapus sticker ${q}!`)
										await sleep(5000)
										} catch (err) {
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`Arya4Me :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
